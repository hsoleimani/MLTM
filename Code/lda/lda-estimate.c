...
{
    // seed (est / inf) alpha k settings data (random / seed/ model) (directory / out)
    ...
    if (argc > 1)
    {
        if (strcmp(argv[2], "est")==0)
        {
            INITIAL_ALPHA = atof(argv[3]);
            NTOPICS = atoi(argv[4]);
            read_settings(argv[5]);
            corpus = read_data(argv[6]);
        ...
        if (strcmp(argv[2], "inf")==0)
        {
            ...
            infer(argv[4], argv[6], corpus);
        }
    }
    else
    {
        printf("usage : lda seed est [initial alpha] [k] [settings] [data] [random/seeded/*] [directory]\n");
        printf("        lda seed inf [settings] [model] [data] [name]\n");
    }
    return(0);
}
