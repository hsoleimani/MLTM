...
for c0, svmC in enumerate(clist):
	...
	runC = [c for c in range(C) if np.sum(trlbl[:,c])>0 and not os.path.isfile('dir/tmp_%d_%d.tar.gz' %(c,c0))]	
...
def test_model_class(c):
	...
	(test_pred[:,c], test_sent_pred[:,c], bags) = testSVM(N, 'dir/tfile_%d' %c, clf)
