...
fpres.write('roc roc_sent wrdlkh roc_macro roc_sent_macro roc_trans roc_macro_trans prob_lbling_error fprAUC traintime\n')
fpres.close()
...
#sentence prediction
slbl_pred = open('%s/testfinal.y' %dirpath).readlines()
...
for doc in slbl_pred:
	...
	for sent in sents_pred:
		if cnt not in slbld_list:
			cnt += 1
			continue
		b[cnt_lbld,:] = np.array([float(x) for x in sent.split()])
		cnt_lbld += 1
		cnt += 1
...
fpres.write('%f %f %f %f %f %f %f %f %f %f\n' %(roc, roc_sent, wrdlkh, roc_macro, roc_sent_macro, roc_trans, roc_macro_trans, prob_lbling_error, fprAUC, trtime))
fpres.close()		
