...
fpres.write('M roc pr roc_sent pr_sent wrdlkh roc_macro pr_macro roc_sent_macro pr_sent_macro fprAUC\n')
fpres.close()
	...
	for general in [0,1]:
		...
		for d,doc in enumerate(slbl_pred):
			...
			for s in range(len(sents)-1):
				...
				'''b_sent[cnt,:] = 0.0
				for j in range(M):
					c = int(j)//int(tpc_per_lbl)
					if c >= C:
						break
					b_sent[cnt,c] += temp[j]
				b_sent[cnt,:] /= float(tpc_per_lbl)
				'''
				cnt += 1
		...
		for c in range(C):
			...
			roc_macroC[c] = metrics.auc(fpr_macro[:,c],tpr_macro[:,c])
			nC += 1
		roc_sent_macro = np.mean(roc_macroC)
